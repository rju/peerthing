...
/**
 * This class manages a form with which you can manipulate the data of a node in
 * the propertyeditor
 * 
 * @author Sebastian
 * @Reviewer Hendrik Angenendt
 */
public class NodeForm implements SelectionListener {
	...
	/**
	 * A label for the name of the node
	 */
	Label renameLabel;

	/**
	 * A textfield filled with the name of the node
	 */
	Text renameText;
	 ...
	 * @param container
	 */
	public NodeForm(Composite container) {

		FormToolkit toolkit = new FormToolkit(container.getDisplay());

		form = toolkit.createScrolledForm(container);
		form.getBody().setLayout(new GridLayout(2, false));
		form.setText("Node Information:");
	...
	/**
	 * Every time the user selects a node in the editor the method will be
	...
	public void update(Object object, PropertyEditor actionview) {
		...
		if (object instanceof SystemBehaviour)
			t1 = ((SystemBehaviour) object).getNodes().get(0);
	...
	public boolean nameCheck(String name) {
		List<INodeType> nodes = t1.getArchitecture().getNodes();
		for (int i = 0; i < nodes.size(); i++) {
			if (nodes.get(i).getName().equals(name)
					&& !nodes.get(i).getName().equals(t1.getName())) {
