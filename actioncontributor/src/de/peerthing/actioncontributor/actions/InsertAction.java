 ...
 * 
 */
public class InsertAction implements IActionExecutor {
	...
	public Object executeAction(IActionSimulator simulator, INode contextNode, ISystemTask task,
		...
		if (destination instanceof List) {
			...
			if (content instanceof IXPathContainer) {
				...
				throw new SimulationException("action insert: Destination is not an XPathContainer.");
		} else
			throw new SimulationException("action insert: Destination is not an XPathObject.");
		if (source != null) {
			if (source instanceof List) {
					...
					for(Object obj : list) {
						if (obj instanceof IXPathAttribute) {
							...
							if (attr instanceof IXPathObject) 
								...
								throw new SimulationException("[" + contextNode.getId() + ":" + task.getId() + "] insert: Attribute content " + attr.getClass().getSimpleName() + " cannot be appended to the content of a container.");
							...
							throw new SimulationException("action insert: XPath result element " + obj.getClass().getSimpleName() + " cannot be appended to the content of a container.");
					}
				}
				...
				if (source instanceof String) { 
					...
					throw new SimulationException("action insert: XPath result " + source.getClass().getSimpleName() + " cannot be appended to the content of a container.");
			}
			return null;
		} else 
			throw new SimulationException("action insert: Parameter source expected.");
	}
